...
def return_model(model: object, allowed_models: list) -> str:
    ...
    return model_types[allowed_models.index(type(model))]
...
def generate_samples(mtype: str, nsamples: int, model_name=None) -> str:
    ...
    if mtype == "GMM":
        ...
        samples = convert_to_df(model.sample(n_samples=int(nsamples))[0], column_names=default_columns)
    else:  # ctgan
        if model_name is None:
            model_name = 'ctgan.pickle'
        model = pickle.load(open(model_name, "rb"))
        samples = model.sample(int(nsamples))
