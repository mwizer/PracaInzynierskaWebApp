...
import asyncio
import numpy as np
from flask import Flask, flash, Response, request, redirect, url_for, render_template, send_file
...
ALLOWED_MODELS = [type(m[0]) for m in MODELS]
...
async def upload():
    if request.method == 'POST':  # handle incoming to app models from users
        ...
        if file and not allowed_file(filename=file.filename, allowed_extension=ALLOWED_EXTENSIONS):
            flash('Niepoprawne rozszerzenie pliku. Dane nie zostały odczytane ani zapisane')
            return redirect(request.url)
        else:
            ...
            if type(check_model[0]) not in ALLOWED_MODELS:
                flash('Zły model')
...
async def sampleModel(name):
    ...
    acc = model[1]*100
    flash(f'Średnia różnica w predykcji klasy względem oryginalnych danych wynosi {np.round(acc, decimals=2)}% (im wynik bliżej 0% tym dane bardziej przypominają rzeczywiste)')
    return render_template('sampleModel.html', modelName=name)


@app.route('/downzip')
async def downzip():
    zip_file_path = 'skrypt.zip'
